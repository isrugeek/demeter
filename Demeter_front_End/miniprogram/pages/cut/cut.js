...
Page({
  ...
  onLoad: function (options) {
    this.setData({ 'imageSrc': options.img_path})
  },
  /**
   * 生命周期函数--监听页面初次渲染完成
   */
  ...
  loadImage: function () {
    ...
    wx.getImageInfo({
      ...
      success: function success(res) {
        DRAW_IMAGE_W = IMG_REAL_W = 512//res.width
        IMG_REAL_H = 512//res.height = 
  ...
  upload: function (path) {
    ...
    var _this = this
    getApp().globalData['tmp_img'] = path
    wx.uploadFile({
      url: getApp().data.servsers + '/reco',
      filePath: path,
      name: 'file',
      formData: {
        'user': 'test'
      ...
      success: function (res) {
        ...
        wx.navigateTo({
          url: '../result/result?result='+JSON.stringify(res_data),
        })
        //do something  
      }, fail: function (err) {
        console.log(err)
